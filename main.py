...
from configs import logging, MIN_PERCENT_REQUIRED, BRL_AMOUNT_TRADE, UPDATE_TICK_RATE
...
initial_balance = last_balance = robot.api.get_balance()
# Calculate the rate limit of request to Biscoint API
endpoints_meta = robot.api.get_meta()
...
ticker = robot.api.get_ticker(base=base)
...
while True:
    try:
        ...
        if calculated_percent >= MIN_PERCENT_REQUIRED:
            ...
            last_balance = robot.api.get_balance()
